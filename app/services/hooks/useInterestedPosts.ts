...
let cachedInterested: { posts: AllProduct[] | null; userId: string | null } = {
  posts: null,
  userId: null,
};
...
export function useInterestedPosts() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (cachedInterested.posts && cachedInterested.userId === profile.id) {
        setPosts(cachedInterested.posts);
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        setPosts(items);
        cachedInterested = { posts: items, userId: profile.id };
      } catch (err) {
  ...
  const cancelInterest = useCallback(
    async (productId: string) => {
      ...
      try {
        ...
        if (cachedInterested.userId === profile.id && cachedInterested.posts) {
          cachedInterested.posts = cachedInterested.posts.filter((p) => p.id !== productId);
        }
      } catch (err: any) {
